...
(defclass <tabular-fn-approx> (<fn-approx>)
  ((params :type hash-table
	   :writer set-params
	   :initarg :params
	   :documentation "The hash table holding parameters")
   (test :type function
	 :initform #'equalp
	 :reader test
	 :writer set-test
   ...
   (default-val :reader default-val
                :documentation "The value of inputs not in the table.  0 by default."
                :initform 0.0
                :initarg :default-val))
		      
  
  (:documentation "Class <tabular-fn-approx>.  
Initargs 
:params -      Hashtable holding the initial value of the fn.  Optional, equal to an empty table
               by default
...
(defmethod reset ((fa <tabular-fn-approx>) &optional theta)
  (if theta
      (progn
	(assert (equal (type-of theta) 'hash-table) ()
                "~A is not a hash-table!" theta)
	(set-params theta fa))
      (set-params (make-hash-table :test (test fa)) fa))
  t)
