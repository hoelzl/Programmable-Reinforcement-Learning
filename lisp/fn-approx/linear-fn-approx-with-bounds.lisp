...
(defclass <linear-fn-approx-with-bounds> (<linear-fn-approx>)
  ...
  (:documentation "Class <linear-fn-approx-with-bounds> (<linear-fn-approx>)
Class for linear function approximation where some bounds on parameters are known.  The online
updates then ensure that these bounds hold at each stage.

Initargs
:mins - a vector.  The Ith entry is either the minimum possible value of the Ith parameter,
        or NIL if no minimum is known.
:maxs - a vector.  The Ith entry is either the maximum possible value of the Ith parameter,
        or NIL if no maximum is known.

Initialization arguments from <linear-fn-approx>:
:dim -    dimension of parameter vector.  
:params - initial parameters.  optional, if provided must be a vector of dimension dim, defaults
          to #(0 0 .... 0)"))

(defmethod initialize-instance :after
    ((f <linear-fn-approx-with-bounds>) &rest args &key mins maxs)
  (declare (ignore args))
...
;;; TODO: Might it be better to just have after methods for <linear-fn-approx-with-bounds> that
;;; take care of enforcing bounds and not repeat the definition of the main mehtods? --tc
(defmethod reset ((fa <linear-fn-approx-with-bounds>) &optional theta)
  (if theta
      (progn
	(assert (equal (length theta) (dim fa)) ()
                "Dimension of ~A is not ~A" theta (dim fa))
	(set-params theta fa))
...
(defmethod update ((fa <linear-fn-approx-with-bounds>) x y eta)
  "Take a step in the direction of the negative gradient of the squared error given this
example"
  (loop
...
(defmethod print-object ((fa <linear-fn-approx-with-bounds>) str)
  (if *print-readably*
      (progn
	(check-exact-class fa '<linear-fn-approx-with-bounds>)
	(format str #. (str "#.(make-instance 'fn-approx::<linear-fn-approx-with-bounds> "
                            ":params ~W :dim ~W :mins ~W :maxs ~W)")
