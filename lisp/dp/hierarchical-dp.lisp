...
(defun inc-prob (l x p comp-fn)
  "inc-prob ALIST ITEM PROB COMPARISON-FN
Assumes ALIST is a sorted association list w.r.t COMPARISON-FN and the values are
real. Destructively modifies ALIST and returns a new one where the value attached to ITEM has
been incremented by PROB (if ITEM did not exist before it is added).  This is used to define
incf-prob.
...
Uses tabular dynamic programming to compute and return two conditional distributions: 
1) the same-level distribution 
2) the exit distribution.

Suppose we are at a state OMEGA with level L.  

The same-level distribution consists of repeatedly sampling from the smdp transition
distribution until reaching another state with level <= L. If this state has level L return it
otherwise return 'nonexistent (which should only happen if OMEGA is an exit state and the next
state has level L-1).

The exit distribution consists of sampling from the transition distribution until we're
at a state with level L-1 and returning it.  If L=0, return 'nonexistent.

The distributions are computed by iterating appropriate Bellman equations (note that their
respective Bellman equations refer to each other)."))
...
(defmethod hierarchical-trans-dists ((m <hierarchical-tabular-smdp>) pol &key (epsilon .01))
  ...
  (let* ((num-states (num-states m))
	...
	((change-pce-to-pc (pce)
	   #.(str "internal function to change pce to pc by replacing transitions to parent-level "
                  "states with 'nonexistent")
	   ;; loop over states
	   (loop 
	       ...
	       ;;; loop over actions at this state
             do (loop
			     ...
			     (loop
                               ...
                               for outcome = (car x)
                               ;; for outcomes at parent level, discard and increment leftover
                               ;; probability
		...
		(if (member diff valid-level-diffs)
		    (incf-outcome-prob l outcome prob)
                    ...
                    (if (member diff diffs-requiring-backup)
                        ...
                        (assert nil ()
                                "Incorrect level difference ~A in backup" diff)))))))))))


(defun recover-outcome-dist (dist m)
  "recover-outcome-dist DIST MDP"
...
(defun initialize-hierarchical-trans-dists (m)						       
  "initialize-hierarchical-trans-dists MDP
Return initial estimates of dists over 
1) next state at same or parent level 
2) next state at parent level"
...
(defun compare-outcomes (o1 o2)
  "compare-outcomes O1 O2
O1 and O2 are outcomes with states being fixnums.  Returns t iff O1 < O2, where comparisons are
done using first state, then reward, then duration."
