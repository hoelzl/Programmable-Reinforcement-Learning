...
(defmethod value-iteration
    ((m <mdp>) &key (epsilon .01) (verbose nil) (init-val) (discount 1.0))
  ...
  (let* ((states (state-set m))
	 ...
	 ;; instead of allocating a new vector on each iteration, allocate two at the beginning
	 ;; (we need two so we can measure the distance between successive iterates)
	 (v (aif init-val
...
(defmethod policy-iteration
    ((m <mdp>) &key (epsilon -1) (k -1) (verbose nil) (init-pol) (discount 1.0))
  ...
  (let* ((states (state-set m))
    ...
    (while (> num-diffs 0)		
      ...
      (setf num-diffs
            (loop 
              ...
              for best-action = (argmax (mapcar #'(lambda (a)
                                                    (backup v i a trans rew discount))
                                                action-inds))
    ...
    (values (policy:make-tabular-policy
             (map 'vector (lambda (x) (item x actions)) pol) states)
	    (value-fn:make-tabular-value-fn v states))))

(defmethod policy-evaluation ((m <mdp>) pol &key (epsilon .01) (discount 1.0))
  (let ((states (state-set m)))
    (value-fn:make-tabular-value-fn 
     (policy-evaluation-inner (transition-matrix m)
                              (reward-matrix m)
                              (pol-to-vector pol states)
                              (make-array (size states)
                                          :element-type 'float 
                                          :initial-element 0.0)
                              discount
                              :epsilon epsilon)
