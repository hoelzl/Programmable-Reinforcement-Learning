(defpackage #:robust-dp
  (:documentation "Package robust-dp
Algorithms for robust dynamic programming as described in the 2004 tech report by Nilim and El
Ghaoui.  - inf-horizon-rvi")
  (:use #:common-lisp
	#:lin-alg
	#:utils
	#:chi-square)
...
(defun inf-horizon-rvi (count-matrix rew-matrix discount unc-level term delta &key successors)
  "inf-horizon-rvi COUNT-MATRIX REW-MATRIX DISCOUNT UNC-LEVEL TERM DELTA &key SUCCESSORS
Implements the algorithm on page 16 of [NG04] for infinite horizon robust value iteration using
the confidence set at UNC-LEVEL given observations in COUNT-MATRIX (a AxSxS array).  The reward
function REW-MATRIX is assumed known.  SUCCESSORS is an array indexed by state, action where
each element is either a list of possible next states or a non-list to indicate that all next
states are possible.  If SUCCESSORS is not provided, all state-action pairs can transition to
all states."
  (loop
    ...
    with succ = (or successors 
                    (make-array (array-dimensions rew-matrix)
                                :initial-element 0))
    with log-lik-gaps = (map-array 
                         (lambda (s) 
                           (compute-log-lik-gap
                            unc-level (if (listp s) (length s) num-states)))
                         succ)
    for k from 0
...
(defun pessimistic-next-step-val (neg-val counts delta log-lik-gaps term succ)
  "pessimistic-next-step-val NEG-VAL COUNTS DELTA LOG-LIK-GAPS TERM SUCC
For each s, a compute a pessimistic next-step expected reward based on the uncertainty set from
COUNTS and LOG-LIK-GAP."
	...
	(setf (aref sigma s a)
              (if (aref term s)
                  ...
                  (multiple-value-bind (count-vector val)
                    ...
                    (- (1+ m) (pessimistic-backup
                               val count-vector delta (aref log-lik-gaps s a))))))))))


(defun pessimistic-backup (v counts delta log-lik-gap)
  "pessimistic-backup V COUNTS DELTA LOG-LIK-GAP
Return the worst-case (where larger values are considered worse) expectation of V under a
probability distribution belonging to the confidence set determined by COUNTS and LOG-LIK-GAP.
V is assumed to be > 0."
	   ...
	   (let* ((freq-vector (map 'vector (lambda (x) (/ x num-samples)) counts))
		  ...
		  (h (l mu)
		    (if (and (> l 0) (> mu v-max))
				...
				unless (= x 0)
                                  sum (* x 
                                         (log (/ (* l x) (- mu y)))))))
                        ;; TODO: Figure out the correct value in this case.  (Originally the
                        ;; code contained a WHEN, so that NIL would be returned if the condition
                        ;; is false.  But since the result of H is subtracted from a number this
                        ;; is almost definitely wrong.  --tc
		 ...
		 (max v-bar
			  ...
			  with mu+ = (let ((z (exp (- beta beta-max))))
				       (/ (- v-max (* z v-bar)) (- 1 z)))
			  ;; initially (debug-print 1 t "~&beta-max = ~a beta = ~a mu- = ~a and
			  ;; mu+ = ~a" beta-max beta mu- mu+)
