...
(defmethod value-iteration ((m <smdp>) &key (epsilon .01) (verbose nil)
  ...
  (recover-value-fn 
   m 
   (value-iteration (prog2
                        (when verbose
                          (force-format t "~&Computing tabular version of smdp..."))
                        (tabular-smdp m)
                      (when verbose
                        (force-format t "done.")))
...
(defmethod policy-iteration
    ((m <smdp>) &key (epsilon -1) (k -1) (verbose nil) (init-pol) (discount 1.0))
  (let ((tab (prog2
...
(defmethod policy-iteration
    ((m <tabular-smdp>) &key (epsilon -1) (k -1) (verbose nil) (init-pol) (discount 1.0))
  (assert (or (> epsilon 0) (> k 0))
          (k epsilon)
          "One of epsilon or k must be provided for policy iteration.")
    ...
    (while (> num-diffs 0)
      ...
      (setf num-diffs
            (loop 
              ...
              for best-action = (do ((best nil) (best-val nil) (j 0 (incf j)))
                                    ((= j num-actions) best)
...
(defmethod policy-evaluation
    ((m <smdp>) pol &key (epsilon .01) (discount 1.0) (k -1) (verbose nil))
  (let ((tab (tabular-smdp m)))
    (recover-value-fn 
     m 
     (policy-evaluation tab (convert-policy m pol)
                        :k k :epsilon epsilon :discount discount :verbose verbose))))
		      

(defmethod policy-evaluation
    ((m <tabular-smdp>) pol &key (epsilon .01) (discount 1.0) (k -1) (verbose nil))
  (smdp-pol-eval-helper
   m pol (make-array (num-states m) :element-type 'float :initial-element 0.0) 
...
Return a value function object from a vector using the state set of SMDP.  V is a vector of
reals."
  (value-fn:make-tabular-value-fn v (state-set m)))

(defun recover-policy (m pol)
  "recover-policy SMDP POLICY
Convert POLICY to a <policy> defined w.r.t. SMDP.  POLICY is a vector indexed by state number,
whose value at state I is the number of the chosen action at state I (numbered w.r.t the
available actions at the state corresponding to I)."
  (let ((states (state-set m)))
...
(defun recover-q-fn (m q)
  "recover-q-fn SMDP Q.
Convert Q to a <q-function> defined w.r.t SMDP.  Q is a vector indexed by state number.  The Ith
value is a real vector indexed by action number (numbered w.r.t the available actions at the
state corresponding to I)."
  (let ((states (state-set m)))
    (q-fn:make-tabular-q-function q states (mapset 'vector
                                                   (lambda (s)
                                                     (avail-actions m s))
                                                   states))))
