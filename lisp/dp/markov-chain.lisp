...
(defun exit-dist
    (trans-dist allowed-states &key (epsilon .01) (test #'equalp) (print-progress nil))
  "exit-dist TRANSITION-KERNEL SET &key (EPSILON .01) (TEST #'equalp) (PRINT-PROGRESS nil)

Return the exit distribution for SET.  That is, a conditional distribution P(s'|s) where s is a
state in SET and s' is the first state not in SET encountered if we start at S and repeatedly
sample from TRANSITION-KERNEL (which is of type [cond-prob-dist]).

EPSILON        is the tolerance parameter for checking convergence.
TEST           is the predicate used to test equality of elements.  If elements can be safely
               compared using #'eq or #'eql, specifying this will speed things up.
PRINT-PROGRESS indicates how many times to print a . indicating progress (convergence measured
               on log scale)

Currently, can loop forever if there's a nonzero probability of staying in the set forever.  In
general, will be quite slow in very random domains with long expected trajectories."
  
  (let* ((num-states (size allowed-states))
	 (dummy-state (gensym))
	 (dists (mapset 'vector (lambda (x)
                                  (declare (ignore x))
                                  (make-hash-table :test test))
                        num-states))
	 (new-dists (mapset 'vector (lambda (x)
                                      (declare (ignore x))
                                      (make-hash-table :test test))
                            num-states))
	 (exit-dist (lambda (s)
	...
	(do-elements (s allowed-states nil i)
	  (let ((new-dist (aref new-dists i)))
	    ...
	    (map-prob-dist (lambda (k v)
                             (declare (ignore v))
                             (setf (gethash k new-dist) 0))
                           new-dist)
