...
(defun make-deterministic-dist (x)
  "make-deterministic-dist X
Return a deterministic distribution over X (represented as an alist)."
  `((,x . 1.0)))
...
(defclass <numbered-set-dist> (<prob-dist>)
  ...
  (:documentation
   "Represents a uniform distribution over a numbered set (maybe later allow arbitrary pf's on
 the set)"))

(defun make-unif-dist-over-set (s)
  "make-unif-dist-over-set SET
Return a uniform distribution over set, which must be of type [numbered-set]."
...
(defun sample-uniformly (s)
  "sample-uniformly SET
Sample uniformly from the finite, numbered set SET."
...
(defclass <rv-dist> (<prob-dist>)
  ...
  (:documentation "Class <rv-dist> (<prob-dist>)
Distribution gotten by composing a random variable with a prob dist.  Can create using
make-rv-dist

Sampling is efficient.  Computing probabilities by default is not so efficient since it just
iterates over the sample space of dist and checks when rv equals the given element."))

(defun make-rv-dist (rv dist)
  "make-rv-dist RAND-VAR PROB-DIST
Compose RAND-VAR with PROB-DIST to get a new PROB-DIST of type <rv-dist>."
...
(defclass <invertible-rv-dist> (<rv-dist>)
  ((inverse :type function
	    :reader inverse
	    :initarg :inverse))
  (:documentation "Class <invertible-rv-dist> (<rv-dist>)
A more efficient subclass of rv-dist for rv's with known inverse.  Create using
make-inverse-rv-dist."))

(defun make-invertible-rv-dist (rv inverse dist)
  "make-invertible-rv-dist RV INVERSE DIST
INVERSE is a function of one argument that either returns the inverse value, or signals a
'rv-inverse-not-defined error."
...
(defmethod prob ((p <invertible-rv-dist>) x)  
  (handler-case
	...
	(let ((z (evaluate-rv (rv p) y)))
	  ;; to be really sure things are working right, we verify that the image of the inverse
	  ;; under rv is the same as the original
	  (assert (same z x) ()
...
(defclass <mixture-prob-dist> (<prob-dist>)
  ((weights :type simple-vector
   ...
   (dists :type simple-vector
	  :reader dists
	  :initarg :dists))
  (:documentation "Class <mixture-prob-dist>
Subclass of <prob-dist> for mixture distributions.  Create using make-mixture-dist."))

(defun make-mixture-dist (weights dists)
  "make-mixture-dist WEIGHT-VECTOR DIST-VECTOR
Create a mixture distribution over the distributions in DIST-VECTOR using weights in
WEIGHT-VECTOR.  Doesn't 'expand out' the new distribution, which means the operations on the
returned distribution take a bit longer."
...
(defclass <sampling-with-replacement> (<prob-dist>)
  ...
  (:documentation "Class <sampling-with-replacement>
A probability distribution that consists of sampling with replacement from some distribution
DIST N times.  Results are collected into a vector of length n.
...
(defmethod sample-space ((p <sampling-with-replacement>))
  (make-instance 'prod-set:<var-set>
    :sets (loop repeat (n p) collect (sample-space (dist p)))))
...
(defclass <sampling-without-replacement> (<prob-dist>)
  ((dist :type [prob-dist] :reader dist :initarg :dist
   ...
   (n :type fixnum :reader n :initarg :n
      :documentation "Number of samples"))
  (:documentation "Class <sampling-without-replacement> (<prob-dist>)
A probability distribution that samples with replacement from DIST N times.  Results are
collected into a vector of length N.

Initargs
:dist
:n

Note that sampling is done just by repeatedly trying to sample each element until we get one
that's different.  So if the DIST assigns most or all of its probability to some M < N elements,
this could go into a long or infinite loop."))
...
(defmethod sample ((p <sampling-without-replacement>))
  (loop
    ...
    do (assert z ()
               #. (str "Can't sample without replacement from dist ~a which has () as a "
                       "sample point") dist)
       (setf (aref a i) z)
...
;;; TODO: expectation not implemented
...
Make a multinomial distribution with probabilities as given by the vector.  V is a vector of
nonnegative reals, and may be unnormalized.  If SET is not provided, it defaults to 0,1,..., N-1
...
(defun make-boltzmann-dist (v temp &optional (domain (length v)))
  "make-boltzmann-dist VEC TEMPERATURE &optional DOMAIN
Create boltzmann distribution from the vector of negative energies.  If DOMAIN is not provided,
it defaults to 0,1,...,N-1 where N is the length of VEC"
  (let ((l (length v)))
    (assert (> l 0) (v)
