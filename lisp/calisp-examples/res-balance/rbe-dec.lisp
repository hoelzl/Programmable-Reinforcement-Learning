...
(defpackage #:rbe-decomposed-crl-q
  (:documentation "rbe-decomposed-crl-q (rbe-dec)
Decomposed crl features for resource balance program.")
  (:use
   #:common-lisp
   #:rbe-prog
   #:calisp-features
...
(defun make-rbe-dec-q-function (max-gold max-wood num-peasants)
  (make-instance '<threadwise-crl-q-function>
    :feature-ids #(gold wood gold-peas wood-peas collision distance carrying?
                   no-goal root-thread peasant-thread)
    ...
    (when (member? i np)
      (labels ((at-nav-choice? (k)
		 ...
		 (let ((pos-i (aref positions i)) (pos-j (aref positions j)))
		   (cond 
                     ;; 1. If they're both at a nav-choice, the feature depends on both of them
                     ((and (at-nav-choice? i) (at-nav-choice? j))
                      (make-calisp-feature 
                       (list i j)
                       (lambda (u v)
                         (indicator (equal (result-legal wm pos-i u)
                                           (result-legal wm pos-j v))))))
                     ;; 2. if only one of them is at a nav-choice, the feature only depends on
                     ;; that one
                     ((at-nav-choice? i)
                      (make-calisp-feature
                       i
                       (lambda (u)
                         (indicator (equal (result-legal wm pos-i u) pos-j)))))
                     ((at-nav-choice? j)
                      (make-calisp-feature
                       j
                       (lambda (u)
                         (indicator (equal pos-i (result-legal wm pos-j u))))))
