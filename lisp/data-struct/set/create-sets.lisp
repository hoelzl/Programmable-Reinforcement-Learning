...
(defun make-image-set (s f &optional (f-inv nil))
  "make-image-set SET FUNCTION &optional (F-INV nil)
Returns a new set which is the image of SET under FUNCTION.  The new set will not be stored in
memory; rather, its members will be computed as needed by applying FUNCTION to elements of SET.
F-INV, if provided, must be the inverse of FUNCTION.  It is optional, but the operations may be
slower if it is not provided."
...
(defun recursive-closure (init-set successor-fn &key (test #'equalp) (print-progress nil) 
  ...
  "recursive-closure INIT-SET SUCCESSOR-FN &key (TEST #'equalp) (PRINT-PROGRESS nil)
                                                  (KEY-FN (constantly 0)) (MAX-KEY 0)

Compute the recursive closure of INIT-SET under SUCCESSOR-FN.  Returns the set (as an
<indexed-set> over a vector).

PRINT-PROGRESS : print a '.' every so-many new elements.
KEY-FN : function that takes in an element and outputs a nonnegative integer.  Used when we have
         prior knowledge about a good hash function for the items.
...
(defun powerset (s)
  "powerset S
Return the set of all subsets of S.  The powerset is not stored explicitly.  The only supported
operations are size, member?, clone, and print-object."
