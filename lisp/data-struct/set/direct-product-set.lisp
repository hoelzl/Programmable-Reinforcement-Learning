...
(defclass <prod-set> (<numbered-set>)
  ((sets :reader sets :writer set-sets :type vector)
   ...
   (inst-accessor :writer set-inst-acc :reader inst-acc
                  :initarg :inst-acc)
   (iterate-quickly :initarg :iterate-quickly :initform nil
                    :reader iterate-quickly)
   (size-products :writer set-size-prods :reader size-prods
                  :type (simple-array * 1)
		  :documentation
                  "The products of successive tails of the dimensions.
For example, if the successive dimensions have cardinality 3, 4, 2, 5 then this would be the
array #(40 10 5 1)") )
  (:documentation 
   "A subclass of set for sets of instantiations to a finite set of variables, each of which can
take a finite set of values.  Uses time, memory proportional to the dimension, not the actual
size (i.e. doesn't actually store all the possible joint values).

Required initargs

:sets -     a sequence of sets.  Variable I ranges over the Ith set in  this sequence.
:inst-acc - an instantiation accessor (see inst-vars package) used to collect together variable
            values into an object.  Must be specified if creating an instance of this class (but
            subclasses may provide it automatically, e.g <var-set>).

If :inst-acc is not provided, the following argument must be provided
:alist-keys - a list of keys, whose length equals the number of sets.  Elements are then
              association lists from these keys.

If :alist-keys is provided :alist-test may also be provided.  Defaults to #'eql.

Optional
:iterate-quickly - nil by default.  If t, then iteration operations (iterator, do-elements,
                   mapset) will work in a more efficient way by not creating a new object each
                   time. This will work ok so long as the calling code does not modify the
                   returned objects, or expect their values to persist (e.g. by saving them in a
                   lexical closure).
...
(defmethod initialize-instance :after ((ps <prod-set>)
                                       &rest args
...
(defmethod clone ((s <prod-set>))
  (make-instance '<prod-set>
    :sets (sets s) :inst-acc (inst-acc s)))
...
Represents an explanation for an item not to be in a product set because of it not being a
well-formed instantiation of the sort required by that product set."))

(defmethod get-explanation-string ((c <invalid-inst>))
  (format nil "The instantiation is invalid for reason ~A."
          (reason c)))
...
Represents an explanation for an item not to be in a product set because one of its components
is not a member of the corresponding component set."))
...
(declaim (inline non-empty-prod-set-iterator))
(defun non-empty-prod-set-iterator
    (s dim sets total-size acc iterators)
  (let ((current-ind 0)
        (inst (item 0 s)))		  
    (lambda ()
                         ...
                         (if done?
                             (let ((iter (iterator (aref sets i))))
                               ...
                               (multiple-value-bind (val done?)
                                   (funcall iter)
                                 (assert (not done?) ()
                                         "iterator unexpectedly finished for set ~a"
                                         (aref sets i))
                             ...
                             (return 
                               (progn
                                 ...
                                 (iterator-not-done inst)))))
                                    ...
                                    "Iteration error for prod set ~a, current item ~a" s inst))))
        (incf current-ind)))))

(defmethod iterator ((s <prod-set>))
  (if (iterate-quickly s)
      ;; if iterating quickly, do a more efficient version that uses the same object each time
      (let ((sizes (sizes s))
	    (dim (dim s))
	    (sets (sets s))
	    (total-size (size s))
	...
	(assert (every #'numberp sizes) ()
                #.(str "Iterator cannot be defined because the coordinate sizes ~a of ~a are "
                       "not all numbers.")
                sizes s)
	(if (> total-size 0)
	    ;; if set is nonempty
	    (let ((iterators (map 'vector #'iterator sets)))              
...
Create a representation of the set of instantiations to the variables in VARS.  The elements of
this set will actually be full joint instantiations from S, but the variables not in VARS will
be ignored, or have their value set to 'not-used.
...
Given an accessor for overall joint instantiations, we want to make a sub-instantiation accessor
for a subset of the variables that can take in instantiations to the entire set of variables,
but treat them as if they were projected down to the subset."
