...
(defun dnf-clauses (f)
  "dnf-clauses F
Return a list of clauses in DNF Formula F.  Does not check that F is a DNF formula."
  (if (is-dnf-clause f)
      (list f)
      (disjuncts f)))
...
(defun clause-literals (c)
  "clause-literals C
Return list of literals in clause C.  If C is t, returns nil.  Asserts if C is nil."
  (assert c ())
  (if (eq c t)
      nil
      (if (is-literal c)
...
(defun make-state-dnf (state props)
  "make-state-dnf STATE PROPS
Make a DNF formula (clause, actually) that is true only for the assignment represented by STATE.
For this to be well-defined (since STATE is specified using the closed-world assumption), we
also need to say what the overall set of proposition symbols is."
  (conjoin-list
   (mapset 'list
           (lambda (p)
             (if (member p state)
...
(defun simplify-dnf (f)
  "simplify-dnf DNF-FORMULA
Check if FORMULA is a DNF.  Then simplify it, by first simplifying all the clauses, then dealing
with trivial clauses and removing the initial or if necessary."
...
(defun inter-clause-simplify (clauses)
  "inter-clause-simplify CLAUSES
CLAUSES is a list of clauses.  Return a DNF formula that is equivalent to the disjunction of the
clauses, with t's and nil's removed"
...
(defun standardize-dnf (f)
  "standardize-dnf FORMULA
Convert FORMULA into a canonical equivalent simplified representation."
...
(defun sort-dnf (f &optional (clone-first t))
  "sort-dnf FORMULA &optional (NONDESTRUCTIVE t)
Assumes FORMULA has already been simplified, and converts it into a canonical representation."
  (when clone-first
    (setf f (clone f)))
  (if (is-dnf-clause f)
...
(defun implies (f1 f2)
  "implies F1 F2
Logical implication.  Only works right now for F1 being a DNF formula and F2 a DNF clause."
...
(defun dnf-and (&rest f)
  "dnf-and &rest DNF-FORMULAE
The simplified logical and of the formulae.  Repeatedly uses the 'distributive law'."
  (case (length f)
    (0 t)
    (1 (car f))
...
(defun dnf-binary-and (f1 f2)  
  "dnf-binary-and F1 F2
The (simplified) logical and of the two formulae.  Uses the 'distributive law'."
...
(defun pprint-dnf-clause (str clause)
  (pprint-logical-block (str (if (is-conjunction clause) (conjuncts clause) clause)
                             :prefix "(" :suffix ")")
    (loop
