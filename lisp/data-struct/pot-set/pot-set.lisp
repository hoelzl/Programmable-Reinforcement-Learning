...
PROD-OP -    the 'multiplication' operation in the underlying semiring.  Should be *times* for
             graphical models, and *plus* for coordination graphs.  See the operation structure
             type.
POTENTIALS - a list of potentials.
ELIM-ORDER - list of variables in order of elimination.  Must be exactly the set of variables
             that potentials depend on.
...
(defun eliminate-and-backsubstitute
    (potentials elim-order s sum-op prod-op back-sub &aux (acc (inst-acc s)))
...
BACK-SUB-FN - a function that takes in a variable number J, and a list POTS of potentials that
              depend on that variable, and returns a function that takes in a partial
              instantiation and returns a value for the variable J.  The returned function may
              not have any side effects on the instantiation, except that the value of variable
              J may be changed.

Eliminates the variables in the given order, then uses the BACK-SUB-FN to reconstruct an
instantiation.  Returns two values : the reconstructed instantiation and its value."
	...
	(flet ((active-ref-pots (j)
	  ...
	  (dolist (v-name elim-order)
		...
		(setf (sbit active-pots i) 0))
	      ...
	      ;; push onto the backsubstitution stack both this variable and a closure that does
	      ;; a step of backsubstitution
...
Return a list of variable names, which represents an elimination order for all variables in
potentials.  The elimination order is chosen according to the minimum deficiency ordering - at
each stage, eliminate the variable whose deletion results in the fewest new edges."
