...
(defstruct (2tbn-var-desc (:conc-name desc-))
  "A var-desc is structure for describing a single (state, action, or reward) variable of a
2TBN.  To create, use make-var-desc, with keywords
...
:prev-slice-parents - list of parents (by name) in the previous time slice (parents of variables
                      are always states or actions)
:curr-slice-parents - list of parents in the current time slice

In addition, for reward variables, include the keyword
:reward-fn - function called on parents values (in a list, in order) to yield this variable's
             value.

In addition, for state variables, include the keywords
:trans-dist -   [cond-prob-dist] of variable given parent values.  Assume parent values are in
                a list, in order.
...
The usual requirement about conditional distributions applies - they should never modify any of
the objects they condition on."
...
Create a description of a state variable that is actually a parameter - that is, it doesn't
change over time.  It's just being included in the state description for convenience.  This is
achieved by making a variable with only a single possible value, namely VAL, and whose init
distribution and trans distributions have no parents and deterministically return VAL."
...
Make a cpd function that takes in a single argument (a list), and destructures it into the vars,
then executes body."
...
(defclass <2tbn> (<cond-prob-dist>)
  ...
  (:documentation "Class <2tbn> (<cond-prob-dist>)
This class represents 2TBNs with actions and rewards.  It can therefore be used to represent
influence diagrams, DBNs, Factored MDPs, etc.

A 2TBN consists of three types of variables - state, action, and reward.  See package
documentation for allowed operations.
...
:state-acc -    instantiation-accessor for state variables (see the inst-vars package
                documentation).  If not specified, defaults to storing state variables in a
                vector.
:reward-acc -   instantiation accessor for reward variables.  If not specified, defaults to
                using a vector.
:action-acc -   instantiation accessor for action variables.  If not specified, defaults to
                using a vector."))

(setf (documentation #'state-set 'function)
      "state-set 2TBN.  Set of instantiations to state variables."
      (documentation #'action-set 'function)
      "action-set 2TBN.  Set of instantiations to action variables.")
...
(defun reward (2tbn prev-s prev-a s)
  "reward 2TBN S A S2
Compute the reward for going from S to S2 after doing A.  The first returned value is the total
reward, and the second is all the reward variables (in whatever format the tbn uses)."
  (loop
...
(defclass <2tbn-init-dist> (<prob-dist>)
  ((2tbn :initarg :2tbn :reader 2tbn))
  (:documentation "Class <2tbn-init-dist> (<prob-dist>)
The initial probability distribution for a 2TBN."))

(setf (documentation 'init-dist 'function)
      "init-dist 2TBN
Return a [prob-dist] representing the distribution of the initial timestep of a 2tbn.")
...
(defmethod initialize-instance :after
  ...
  (let ((descs (map 'vector (lambda (x)
                              (if (listp x)
                                  (apply #'make-2tbn-var-desc x)
                                  (clone x)))
