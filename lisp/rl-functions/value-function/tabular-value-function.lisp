...
(defclass <tabular-value-fn> (<value-fn>)
  ...
  (:documentation "Class <tabular-value-fn> (<value-fn>)
A <tabular-value-fn> consists of a vector VALS of values, and a numbered set S.  The value of X
is gotten by looking up the number of X in S, and getting the corresponding item from the vector
VALS."))

(defun make-tabular-value-fn (v &optional (s (length v)))
  "make-tabular-value-fn VALS &optional STATE-SET.
Makes a <tabular-value-fn> having the given values.  If SET is not given, it is assumed to be
the set 0,1,...,N-1 where N is the length of VALS."
...
(defmethod print-object ((v <tabular-value-fn>) str)
  (print-unreadable-object (v str :type t)
    (format str "state set ~A, values ~A" (state-set v) (vals v))))

;;; TODO: Remove explicit package reference.  --tc
