...
A particular kind of decomposed Q-function in which each component is a CRL Q-function, and the
components share a weight vector.  Additionally, the overall Q-function also works like a CRL
Q-function, in that it uses a coordination graph to pick the best choice at a state.

Initargs
:feature-ids - vector of names of feature templates

Provide at most one of
:decomposed-templates -      a vector of decomposed templates, each of which is a function that
                             takes in two arguments - a joint state and component id - and
                             returns a designator for a list of features
:decomposed-template-descs - analogous to the parameter :template-descs of <crl-q-function> - a
                             list of descriptions, each of which is a designator for a list of
                             decomposed feature templates"))
...
(defmethod shared-initialize :around 
    ((q <decomposed-crl-q-function>) slots 
     &rest args 
     &key (decomposed-template-descs nil supplied)
          (feature-ids (coerce 
...
(defmethod shared-initialize :after ((q <decomposed-crl-q-function>) slots &rest args)
  ...
  ;; a decomposed-crl-q-function is, among other things, a crl-q-function, and must have a set
  ;; of feature templates, so next compute the set of templates from the set of decomposed
  ;; feature templates and set the slot appropriately.
