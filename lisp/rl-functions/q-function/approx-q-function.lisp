...
(defclass <approx-q-function> (<q-function>)
  ...
  (:documentation "Class <approx-q-function>
Subclass of <q-function> that uses an object of type <fn-approx> to represent the Q-function,
and doesn't assume any structure in the set of available choices at a state.
...
Initargs
FN-APPROX:       the function approximator.  Defaults to a tabular function approximator based
                 on #'equalp.
FEATURIZER:      function of two arguments, state and choice, that returns an object
                 representing the features for this state and choice.  Defaults to
                 #'canonicalize.
FEATURIZER-NAME: a symbol, such there is a globally defined variable with this name that
                 evaluates to the featurizer function.  If not provided, and featurizer equals
                 the default featurizer, then it is automatically set.  Otherwise, it is left
                 unset.  In the latter case, the object can not be written to a stream in a
                 readable format.  To read the object back from the stream, it must be the case
                 that the featurizer is still defined (so you can't save a Q-function to a file,
                 and then just restart Lisp and try to read it in - you have to load the Alisp
                 system code and the code that defines the featurizer first).
...
Print to STREAM a string that, when read, will result in the creation of a new object of the
same type as Q-FN, which also has the same function approximator and featurizer as Q-FN.  For
this to work, the featurizer-name field of Q-FN has be non-nil (if Q-FN was created using the
default featurizer, this will happen automatically), and all the subcomponents have to
themselves be printable readably."
...
(defgeneric featurize (q-fn omega u)
  (:documentation "featurize APPROX-Q-FN OMEGA U
Return the ``feature-vector'' (which need not actually be a Lisp vector) for this state and
choice, which is then passed to the function approximator.  The default method just calls the
function object stored in the featurizer slot of q-fn (which in turn, by default
returns (canonicalize (cons omega u))).")

  (:method ((q-fn <approx-q-function>) omega u)
    (funcall (featurizer q-fn) omega u)))
...
;;; TODO: Try to get rid of the dependence on GET-LAMBDA-LIST.  (Maybe introduce a second
;;; argument that specifies the number of parameters?) --tc
...
- A function F of two arguments, denoting itself."
  ...
  (if (functionp f)
      (let ((l (get-lambda-list f)))
        ;; SBCL, CCL and ECL return lambda-list '() for some functions
        ;; that take an arbitrary number of arguments, e.g.,
        ;; (constantly 1).  This hack fixes that until I find a better
        ;; solution. --tc
        (if (null l)
            #+(or sbcl ccl ecl)
...
FEATURES:    List of designators for Q-features.  See (help get-designated-q-feature) for
             details."
