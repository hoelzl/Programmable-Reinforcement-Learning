...
(defclass <prompt-policy> (<policy>)
  ...
  (:documentation "Class <prompt-policy> (<policy>)
A <prompt-policy> is one which makes choices by asking the user.  It also has a list of
'advisors', which can print their advice before the user enters a choice.
...
:io-specifier - An object that governs how the IO is done when asking the user to enter a
                choice.  Nil by default, which amounts to just printing the state and set of
                choices, and using the Lisp reader to interpret what the user enters as an
                action.  This will work fine in almost all situations.  Modifying this behaviour
                requires providing a method for prompt-for-choice"))
...
(defmacro do-upto-max-choices ((choice-var) &body body)
  "do-upto-max-choices (CHOICE-VAR) &rest BODY
Do BODY once for each choice, upto *max-num-choices-to-print*"
...
(defgeneric prompt-for-choice (s choices io-spec)
  (:documentation "prompt-for-choice STATE CHOICES IO-SPEC
A way to customize the IO behaviour of prompt for choice.  Methods for this generic function are
expected to prompt the user for a choice, parse the entered choice, and return it.  The process
may involve a back-and-forth interaction with the user.  The method may also signal a
'choose-to-abort error if the user enters some value that signifies that the io-interface should
finish.
...
- If it's not a member of the choice set, and if the set of choices has size 1, then prints a
  warning message and returns that single member.
- If it's not a member of the choice set, and if the set of choices has size greater than 1,
  asserts and allows the user to enter another choice.
...
- alisp:io-interface and calisp:io-interface use a prompt policy with io-spec being the partial
  program.")
...
(defgeneric print-advice (advisor omega choices str)
  (:documentation "print-advice ADVISOR STATE CHOICES STREAM
Given that we are at STATE and have choices CHOICES, print ADVISOR's opinion to STREAM."))
...
(defun print-cond-dist-advice (adv omega choices str)
  "prind-cond-dist-advice ADVISOR STATE CHOICES STREAM
Print information about predicted outcomes of user choices."
  (format str #.(str "Function of one argument being treated as a conditional "
                     "distribution by prompt-policy."))
  (set:do-elements (u choices)
...
(defun print-state-action-feature-advice (adv omega choices str)
  "print-state-action-feature-advice ADVISOR STATE CHOICES STREAM
Print information about the features resulting from user choices."
