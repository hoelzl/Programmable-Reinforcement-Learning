...
(defclass <random-policy> (<policy>)
  ((choice-fn :type function :initarg :choice-fn :reader choice-fn :writer set-choice-fn))
  (:documentation "Class <random-policy> (<policy>)
A policy that chooses uniformly at random from the actions available at any state.  Assumes the
environment implements avail-actions, and that the action set is small enough that we can sample
...
(defun make-random-policy (choice-fn)
  "make-random-policy CHOICE-FN
Make a policy of type <random-policy> that chooses randomly from the set of actions available at
a given state."
