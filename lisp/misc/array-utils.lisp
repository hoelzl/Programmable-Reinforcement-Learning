...
(defun eql-array (a1 a2)
  "eql-array A1 A2
Return true iff arrays have the same size and corresponding elements are eql."
...
(defun round-array (a d)
  "round-array A D
Return a new array in which all elements of array A are rounded to D decimal places (usually
used for displaying).  Coerces all elements to floats in the process.  Inefficient
implementation intended only for debugging."
  ...
  (if (numberp a1)
      ...
      (if (numberp a2)
          ...
          (let* ((d1 (array-dimensions a1))
                 ...
                 ;; TODO: Fix this --tc
                 ;; not the right way to do it (and inefficient since a string is being created
                 ;; each time).  should signal a condition instead.
                 (dim-err (format nil
                                  "Array dimensions ~a and ~a do not match in a*"
                                  d1 d2)))
            (cond ((and (= (length d1) 1) (= (length d2) 1))
                  ...
                  ((and (= (length d1) 2) (= (length d2) 2))
                   (assert (= (second d1) (first d2)) nil dim-err)
                   (loop
                     with prod = (make-array (list (first d1) (second d2))
                                             :element-type 'float)
                     for i below (first d1)
                     do (loop
			  for j below (second d2)
			  do (setf (aref prod i j)
                                   (loop for k below (first d2)
                                         summing (* (aref a1 i k)
                                                    (aref a2 k j)))))
                     finally (return prod)))
...
(defun 1s (&rest dims)
  "1s &rest DIMS
Make an array of element-type float with dimensions DIMS containing all 1s."
  (make-array dims :element-type 'float :initial-element 1.0))

(defun 0s (&rest dims)
  "0s &rest DIMS
Make an array of element-type float with dimensions DIMS containing all 0s."
  (make-array dims :element-type 'float :initial-element 0.0))


(defun array-size-multipliers (a)
  "array-size-multipliers A
Return the multipliers needed to step through the various dimensions of the array.  For example,
an array with dimensions 3,5,4,2 would have multipliers 40,8,2,1"
  (loop
    with step = 1
    with mults = nil
...
(defun subarray (a inds)
  "subarray A INDS
Returns an array consisting of all entries of A whose index begins with INDS (which is a list).
E.g., if A has dimensions 2,3,4,3 then subarray(a,(1 2)) will return the 4x3 array
a[1,2,0,0],...,a[1,2,3,2].  Note that the new array shares structure with the original one,
i.e. if the entries are modified, then the original array will be changed as well."
  (let* ((offset 0)
	 (newdims
           (loop
             with indices = inds
...
(defun reshape-array (a dims &key (clone-p nil))
  "reshape-array A DIMS &key (CLONE-P nil)
Reshape array to new dimensions DIMS (which must have the right total number of elements).  If
CLONE-P is true, then items are cloned."
  (let ((old-dims (array-dimensions a)))
    (assert (equal (apply #'* dims) (apply #'* old-dims))
            () "Array of size ~a cannot be converted to array of size ~a!"
            old-dims dims))
  (loop
...
(defun sparsify (v &key test (name-fn #'identity))
  "sparsify V &optional TEST NAME-FN
Return a vector w each of whose elements is a pair (ind . val), which means that v(ind) = val,
for all the nonzero elements of v.  w is in increasing order of ind.  TEST is used to select
which elements are included.  By default, all elements that are nonzero are included.  NAME-FN
is applied to ind before printing it.  By default, its the identity function."
  (loop
