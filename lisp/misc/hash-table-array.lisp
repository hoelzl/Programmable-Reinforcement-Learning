...
(defpackage #:hash-table-array
  (:documentation
   "Defines a data type for arrays of hash tables.
...
Make an array of hash tables using KEY-FN, which maps objects to nonnegative integers.  The
individual hash tables use TEST as the test."
...
(defun get-val (x hta)
  ...
  (gethash (canonicalize x)
           (aref (hta-a hta) (funcall (hta-key-fn hta) x))))

(defun set-val (x hta new-val)
  "set-val KEY NEW-VAL HTA.  Like (setf (gethash KEY H) NEW-VAL)."
  (setf (gethash (canonicalize x)
                 (aref (hta-a hta) (funcall (hta-key-fn hta) x)))
        new-val))
