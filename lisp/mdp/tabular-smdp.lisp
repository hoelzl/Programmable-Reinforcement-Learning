...
(defclass <tabular-smdp> (<smdp>)
  ((termination-vector 
    :type (vector boolean)
    :writer set-term-vec :reader term-vec)
   (num-avail-actions-vector
    :type (vector fixnum)
    :writer set-num-avail-actions-vec :reader num-avail-actions-vec)
   (original-smdp
    :type <smdp>
    :initarg :smdp :initform (required-initarg :smdp)
    :reader smdp)
   (smdp-trans
    :type (array * 1)
    :writer set-trans-vec :reader trans-vec))
  (:documentation "Class <tabular-smdp> (<smdp>)
An SMDP where states and actions are represented as fixnums, and trans-dist, term-pred, and
avail-actions are all represented with vectors.  Used as a more efficient internal
representation by the dynamic programming algorithms.

Create from an SMDP using function tabular-smdp"))
...
;;; TODO: The caching should be done in a method on MAKE-INSTANCE. --tc
(defgeneric tabular-smdp (smdp)
  (:documentation "tabular-smdp SMDP
Return a tabular smdp equivalent to SMDP.  There is an around method that first checks if there
is a cached copy of it already stored, and otherwises calls the next method and caches the
result.")
  (:method :around (smdp)
