...
(defmethod clone ((pe <hash-exit-distribution>))
  (make-instance '<hash-exit-distribution>
    :featurizer (featurizer pe)
    :cond-dists (copy-hash-table (cond-dists pe) :value-copy-fn #'clone-dist)))


(defgeneric update-exit-dist (exit-dist omega u new-dist eta)
  (:documentation  "update-exit-dist EXIT-DIST OMEGA U NEW-DIST ETA
Move the exit distribution at OMEGA,U closer to NEW-DIST by ETA, unless NEW-DIST is unknown.  If
OMEGA,U hasn't been seen before, pretend that its current distribution assigns probability 1 to
'DUMMY-UNKNOWN-STATE.")
...
(defun renormalize (dist)
  "renormalize DIST
Renormalize a distribution to get rid of the unknown state, unless that's the only state.  Note
this implicitly relies on the distribution (which originally comes from MAKE-DETERMINISTIC-DIST
or UPDATE-DIST) being a list."
