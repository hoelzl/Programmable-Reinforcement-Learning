...
Evaluate BODY with various lexical bindings in effect.  JS-VAR-NAME is assumed bound in the
surrounding context to an alisp joint state.
...
Return a function of two arguments named STATE-VAR-NAME and CHOICE-VAR-NAME that evaluates BODY
with a set of additional lexical bindings in effect (see use-alisp-state-bindings)."
...
(defmacro def-feature (name (omega u) &rest args)
  "def-feature NAME (STATE-VAR-NAME CHOICE-VAR-NAME) [DOC-STRING] &rest BODY
Like defun, except certain lexical bindings are available within BODY.  See make-feature for
details."
  (multiple-value-bind (arglist body)
      (expand-make-feature omega u args)
...
CP-SPEC   (unevaluated) is an symbol.
          Each element of Q-COMP-SPECS is a list (unevaluated) of the form
          (COMP-NAME &rest FEATURES)
...
Each element of FEATURES is either a symbol, which designates a function with that name, or a
nonsymbol, which designates the function it evaluates to.

The macro overall expands to code that returns a list containing three elements, namely the
featurizers for Qr, Qc, and Qe.  Each featurizer is a function that takes in a joint state and
choice.  The function first looks at the label of the choice point and selects a CLAUSE whose
CP-SPEC is equal to this label.  It then selects the appropriate Q-COMP-SPEC (the one having the
right COMP-NAME).  Finally, it applies each of the features in this Q-COMP-SPEC to the state and
choice, and collects the values into a vector.
