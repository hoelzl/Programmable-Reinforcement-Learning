...
(defclass <stat-gatherer> (<alisp-observer>)
  ...
  (:documentation "CLASS <stat-gatherer> (<alisp-observer>)
Class for gathering stats about a run of alisp.  Resets all counts the run function is called by
default.  Right now counts total reward, num episodes started, num episodes ended, num env
steps, num choice states.  Can add more stats in future, but make sure everything is O(1)
space (so we can include this class as a default observer and not worry about memory).
...
(defmethod print-object ((obs <stat-gatherer>) str)
  (format str "~&ALisp stats
~A episodes started and ~A finished
~A env steps and ~A joint choices"
	  (num-episodes-started obs) (num-episodes-finished obs)
          (num-env-steps obs) (num-joint-choices obs)))
