...
(defclass <rlm> ()
  (
   ...
   (env
    :type env-user:<env>
    :initarg :env :initform (required-initarg :env)
    :reader env
    :documentation "The environment in which the RLM operates.")
   (partial-program
    :type [alisp-program]
    :initarg :part-prog :initform (required-initarg :part-prog)
    :reader part-prog
    :documentation "The partial program executed by this RLM.")
   (observers
    :initarg :observers
    :reader observers :writer set-observers
    :documentation "The observers for this RLM (including all learning algorithms).")
   (policy
    :type policy:[policy]
    :initarg :policy :initform (required-initarg :policy)
    :reader policy
    :documentation "The policy followed by the RLM.")
   ...
   (num-steps
    :type (or null fixnum)
    :initarg :num-steps :initform nil
    :reader num-steps
    :documentation
    "The maximum number of total steps the RLM can run before quitting.")
   (num-episodes
    :type (or null fixnum)
    :initarg :num-episodes :initform nil
    :reader num-episodes
    :documentation "The number of episodes the RLM should run.")
   ...
   (elapsed-steps
    ...
    :documentation "The total number of steps elapsed during the run of the RLM.")
   
   (elapsed-episodes
    :type fixnum
    :initform 0
    :accessor rlm-elapsed-episodes
    :documentation "The episodes completed in the current run of the RLM.")
   
   (joint-state
    :initform (make-joint-state)
    :reader joint-state :writer set-joint-state))
   
   
  (:documentation "Class <rlm>
Class that basically serves as a place to put all the relevant local state while running an ALisp program.  Required initargs are
:env -       <env>
:part-prog - <part-prog>
:policy -    <alisp-policy>
...
(defmacro notify-all-observers (rlm msg &rest args)
  "notify-all-observers RLM MSG &rest ARGS
Send MSG applied to ARGS to all observers of RLM."
  (with-gensyms (obs)
...
(defgeneric finish-episode (rlm)
  (:documentation "finish-episode RLM
Called when an episode reaches a final state.  The default implementation notifies all observers
with the message INFORM-PART-PROG-TERMINATED."))
...
Resets the elapsed steps and episodes to 0, notifies all observers with INFORM-START-EXECUTION,
repeatedly initializes a new episode, executes the partial program of the RLM and calls
FINISH-EPISODE.  The loop ends when a condition of type RLM-LAST-STEP-REACHED or CHOOSE-TO-ABORT
is signalled; after finishing the loop all observers are informed that the execution of RLM has
stopped by sending the message INFORM-FINISH-EXECUTION.  Each episode can be aborted by
signalling a condition of type ENV-TERMINATED, in which case FINISH-EPISODE is called on the
RLM, and either a new episode is started or, if more episodes than (NUM-EPISODES RLM) have been
run, the loop is exited by signalling RLM-LAST-STEP-REACHED."
  
  ;; during this run, dynamically bind the *rlm* special variable to rlm for debugging purposes,
  ;; for now just setf *rlm*
  ...
  (handler-case      
      ;; loop over episodes
      ;; The loop is exited when an rlm-last-step-reached condition is signalled
      (loop
	(initialize-episode rlm)
	;; this form exits when the environment terminates or an rlm-last-step-reached error is
	;; signalled
	(handler-case
            ;; TODO: Should we not call FINISH-EPISODE and CHECK-LAST EPISODE when an episode
            ;; terminates normally? --tc
	    (start (part-prog rlm))
	  (env-terminated ()
	    (finish-episode rlm)
...
;;; TODO: Fix packages so that env-user:-prefix is not necessary.  --tc
...
(defmethod finish-episode ((rlm <rlm>))  
  ...
  (assert (env-user:at-terminal-state (env rlm)) ()
...
LABEL (not evaluated) is the label for this location in the program."  
  ...
  (with-gensyms (rlm omega act-var old-s reward new-s term num-steps new-frame cloned-state)        
    `(let ((,rlm *rlm*)
       ...
       ;; TODO: It might be more efficient to defer cloning.  Create a closure that clones the
       ;; state and stores the result, and pass that closure to INFORM-ALISP-STEP. --tc
	 ...
	 (assert (not (env-user:at-terminal-state (env ,rlm))) ()
...
(defun env-state ()
  "get-state.  
Return the environment state.  The returned value should be treated as immutable, since the
environment might be holding a reference to it."
...
LABEL   (not evaluated) is the label of this location in the partial program.  Optional, and if
        omitted, label defaults to FN-NAME.
...
ARGS    (not evaluated) is a lambda list, in which some elements may be of the form
        (choose-arg CHOICE-LIST) where CHOICE-LIST is evaluated.  These parameters are called
        `unspecified'."
  (condlet
      (((= 1 (length arg-list))
        (label (caar arg-list))
        (fn-name (caar arg-list))
        (args (cdar arg-list)))
       (t (label (car arg-list))
          (fn-name (caadr arg-list))
          (args (cdadr arg-list))))
    (let ((num-unspecified 0)           ;; how many vars are unspecified
          (choice-lists '())            ;; list of choices for each unspecified param
          (unspecified                  ;; vector where nth element is t if nth param unspecified
            (make-array 0 :adjustable t :fill-pointer 0))
          (unspecified-param-nums       ;; vector of numbers of unspecified params
            (make-array 0 :adjustable t :fill-pointer 0))
      ...
      (with-gensyms (rlm new-frame ch c choice-list-vals 
        ...
        `(let ((,rlm *rlm*)
           ...
           ;; update the program counter and frame label
           (update-program-counter ,rlm ',label)
           (setf (frame-label (first (js-stack ,omega))) ',label)
...
CHOICES: (not evaluated) list, where either all choices are of the form 
         ...
         statement. except that the choose-arg construct may not be used for any of the
         arguments"
...
KEY  (not evaluated) is the key to store it under.  If not provided, defaults to ITEM (not
     evaluated)
...
ITEM (evaluated) is the object to be stored
KEY  (not evaluated) is the key to store it under.  If not provided, defaults to ITEM (not
     evaluated), assuming it's a symbol.
...
(defun choose-using-completion (rlm omega)
  "choose-using-completion RLM STATE
Use RLM's policy to choose a value.  If STATE is unknown the sample uniformly from all available
choices in STATE."
  (handler-case
...
(defun update-program-counter (rlm label)
  "update-program-counter RLM LABEL
Update the program counter of RLM.  LABEL is the label of the generalized choice point that the
program is at.  Uses the value of *containing-subroutine-name* as the containing subroutine."
  (setf (js-pc (joint-state rlm)) 
        (make-program-counter
         :containing-subroutine *containing-subroutine-name* :label label)))

(defun lookup-alisp-subroutine (f)
  (let ((l (get-lambda-list f)))
    (assert (notany (lambda (x) (member x '(&optional &key &rest &aux)))
                    l)
            () 
