...
(defpackage #:alisp-smdpq
  (:nicknames #:asq)
  (:documentation "Package alisp-smdpq.
The smdp q-learning algorithm, applied to alisp. 
...
(defclass <smdpq> (<alisp-learning-algorithm> <q-learning-algorithm>)
  ...
  (:documentation "Class <smdpq>
Subclass of <alisp-learning-algorithm> that uses SMDP Q-learning.
...
Make an instance of smdp q-learning.  If Q-FN not provided, use tabular rep."
  (make-instance '<smdpq>
    :lrate lrate :disc discount :q-fn q :debug-str debug-stream :hist-out-dir hist-out-dir))
...
(defun smdpq-finish-episode (alg)
  ...
  (let ((prev-omega (prev-omega alg))
    ...
    (when prev-omega
      ...
      (debug-msg alg
                 #.(str "~&Episode terminated.  Backup to state ~A and action ~A "
                        "with old value ~A towards new value ~A.")
  ...
  (unless (action-state? omega)
	...
	(handler-case
	    (let ((new-val (+ (rew alg)
	      ...
	      (debug-msg alg
                         #.(str "~&Backup to state ~A and choice ~A with old value ~A "
                                "towards new value ~A.")
