...
(defpackage #:alisp-hordq
  (:nicknames #:ahq)
  (:documentation "Package alisp-hordq.
Hierarchically optimal recursively decomposed Q-learning for ALisp.
...
(defstruct csi
  "Structure type CSI (Choice-stack item).  
Contains bookkeeping information regarding the last past joint choice at some level of the
choice stack, for which backups may be done in the future."
...
(defmethod print-object ((c csi) str)
  (print-unreadable-object (c str :type t)
    (format str "with omega = ~A~&u = ~A visited ~A qr-backup-flag ~A "
            (csi-omega c) (csi-u c) (csi-visited c) (csi-qr-backup-flag c))
    (format str "~&discount ~A lrate ~A~&accumulators ~A"
...
(defclass <hordq> (<alisp-learning-algorithm> <q-learning-algorithm>)
  ...
  (:documentation "Class <hordq> (<alisp-learning-algorithm> <q-learning-algorithm>)
Does hierarchical q-learning, using the 3-part decomposition Q = Qr + Qc + Qe.
...
FEATURES:  if provided, this must be a list with three elements, namely the featurizers for Qr,
           Qc, and Qe."))
...
(defmethod print-object ((alg <hordq>) str)
  (print-unreadable-object (alg str :type t)
    (format str "with ~A choice stack items and ~A active exit points"
...
(defun backup (c q-fn rew-acc final-val)
  "backup CSI Q-FN REW-ACC FINAL-VAL
Do a backup for the given q function and choice-stack item, assuming that REW-ACC units of
discounted reward have been received, and the future reward is estimated by FINAL-VAL"
