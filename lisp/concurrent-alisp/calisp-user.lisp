...
(defvar *io-interface-debug-threads* t
  "T by default for now.  If true, then running the io-interface also prints low-level
information about thread-related events to standard output.")
...
Run this partial program in the environment in a mode where events in the program are printed
and choices are made by asking the user.  For any of the observers in OBS that are also learning
algorithms, set their debug stream to *standard-output* beforehand.  Store history in *hist*"
  (let ((pol (make-instance 'policy:<prompt-policy>
    ...
    (let ((old-vals (mapcar 
		     #'(lambda (obs)
                         (when (typep obs '<calisp-learning-algorithm>)
                           (debug-str obs)))
		     alg-list)))
...
(defun learn (part-prog env policy obs num-steps
  ...
  "learn PART-PROG ENV POLICY OBSERVER-LIST NUM-STEPS
      &key (HIST-LENGTH nil) (EPISODE-PRINT-INC 10) (STEP-PRINT-INC 100)
Run ALisp partial program in environment using POLICY to make decisions.  POLICY can also be the
symbol 'random for random choices.  Note that the learning algorithms are not reset beforehand -
if you want them to start from scratch, call reset on them first."
  (setf policy
        (case policy
...
(defun on-policy-learning (part-prog env alg &rest args)
  "on-policy-learning PART-PROG ENV ALG &rest ARGS
Like calling (learn PART-PROG ENV ALG ALG &rest ARGS)."
...
Front end for evaluating completions of a CALisp partial program, represnted as policies.
POLICIES is either a policy or a numbered set of policies.  Do the following for each policy :
reset environment to a random state, and run the given policy, computing the total reward, until
either the episode terminates or NUM-STEPS environment steps happen.  Repeat this NUM-TRIALS
times.  If POLICIES is a <numbered-set>, return a corresponding sequence of average total
rewards rounded to 2 decimal places.  If POLICIES is not a numbered-set, just return a
number. Acknowledgement is printed every STEP-PRINT-INC steps, if it is non-nil.  Future todo :
make it so a set of initial states is sampled, and those initial states are used to evaluate
each policy."
