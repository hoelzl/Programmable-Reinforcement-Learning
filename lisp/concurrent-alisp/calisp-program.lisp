...
ASSIGN-EFFECTORS-FN: function that takes in a joint state and the set of unassigned effectors
                     and returns an alist of which thread each one should be added to.  By
                     default, all new effectors are assigned to the root thread.
CHOOSING-THREAD-FN:  how the set of choosing threads is selected.  By default, equals 'all.  The
                     allowed values for now are 'all, 'highest-level-first,
                     'highest-level-single, or a function"))
...
(defgeneric choosing-thread-ids (cpp omega)
  (:documentation "choosing-thread-ids CPP OMEGA
Return list of ids of threads that are going to choose next in OMEGA.")
...
(defun ordered-choosing-threads-function (order)
  "ordered-choosing-threads-function ORDER
Return a choosing-threads function that returns the list of all threads whose label is earliest
in the ordering."
  (lambda (omega)
...
FUNCTION-NAME to the current environment state of a concurrent ALisp program."
  `(defun ,feature-name ()
     ,doc-string
     (,fn-name (env-state))))
