...
(defclass <crlm> ()
  (
   ...
   (part-prog-terminated?
    :type boolean
    :documentation
    "True iff the partial program has terminated without the environment terminating."
   ...
   (actions 
    :documentation
    "Table that holds the joint action as it is built up.  Maps effector to individual action"
    :type hash-table
    :initform (make-hash-table :test #'eq)
    :reader actions)
   ...
   (reassign-list
    :documentation
    "List of effector-thread pairs that represent reassignments that must happen at the
beginning of the next env step."
  ...
  (:documentation "Class <crlm>
Class that basically serves as a place to put all the relevant local state while running a
concurrent ALisp program.  Required initargs are
...
(defmethod initialize-instance :after ((c <crlm>) &rest args)
  ...
  (let ((lock (lock c)))
    (set-action-condition
     (make-instance '<condition-variable> :lock lock :name "action-cv") c)
    (set-choice-condition
     (make-instance '<condition-variable> :lock lock :name "choice-cv") c)
    (set-wait-condition
     (make-instance '<condition-variable> :lock lock :name "wait-cv") c)
    (set-effector-condition
     (make-instance '<condition-variable> :lock lock :name "effector-cv") c)
    (set-step-condition
     (make-instance '<condition-variable> :lock lock :name "step-cv" ) c)))
...
(defmacro notify-debugging-observers (crlm msg &rest args)
  "notify-debugging-observers CRLM MSG &rest ARGS
Notify debugging observers of MSG.  The joint state is also cloned and inserted before the
ARGS."
...
(defmacro notify-all-observers (crlm msg &rest args)
  "notify-all-observers CRLM MSG &rest ARGS
Notify all observers (including debugging observers) of MSG with ARGS."
  (with-gensyms (obs)
    `(progn
...
(defun start-episode (c &aux (root-thread-id (root-thread-id (part-prog c))))
  "start-episode C 
Reset environment state, create root thread, and set up bookkeeping vars."
...
(defun joint-action (c &aux (env (env c)))
  ...
  (let* ((actions (actions c))
	 (act (mapset 'list (lambda (eff) (cons eff (gethash eff actions)))
                      (current-effectors c)))
	...
	(let ((src-id (check-not-null (gethash eff eff-t-ids)
	  ...
	  (let* ((src-state (check-not-null (gethash src-id (thread-states c))
	    ...
	    (assert (member eff src-effectors :test #'equal) ()
                    #.(str "Unexpectedly, effector ~A did not belong to effector list ~A "
                           "of thread ~A before reassign.")
...
(defun handle-effector-changes (c old-effs new-effs)
  "handle-effector-changes CRLM OLD-EFFS NEW-EFFS
Do the necessary bookkeeping to figure what effectors are new/removed, and call the partial
program's assign-new-effectors method to figure out which thread gets the new effectors."
  (multiple-value-bind (added deleted)
	...
	(mapc (lambda (eff id)
                (multiple-value-bind (t-state exists?)
                    (gethash id t-states)
                  (assert exists? ()
                          #.(str "Attempted to assign new effector ~A to nonexistent "
                                 "thread ~A.  Actual set of threads is ~A.")
...
ARGS:    (not evaluated) is a lambda list, in which some elements may be of the form
         (CHOOSE-ARG CHOICE-LIST) where CHOICE-LIST is evaluated."
  (let ((num-unspecified 0)             ;; how many vars are unspecified
	...
	(unspecified                    ;; list where nth element is t if nth param unspecified
          (make-array 0 :adjustable t :fill-pointer 0))
	(unspecified-param-nums         ;; list of numbers of unspecified params
          (make-array 0 :adjustable t :fill-pointer 0))
	(exit-label                     ;; label of exit point of this call
          (intern-compound-symbol label "-EXIT"))
    ...
    (with-gensyms (c new-frame choice param-num param-choice lock current-thread-id
      ...
      `(let ((,c *crlm*)
	     ...
	     ,@(map 'list (lambda (a v u) `(,v ,(if u ''unspecified a)))
                    args arg-vars unspecified))
	 ...
	 (with-process-lock (,lock)
	   ...
	   (multiple-value-bind (,current-thread-id ,ts)
		 ...
		 ,(case num-unspecified
		    ...
		    (1 `(set-frame-var-val ,new-frame
                                           (nth ,(aref unspecified-param-nums 0) ,param-names)
                                           ,choice
                                           t))
		    (t `(map nil (lambda (,param-num ,param-choice)
				   (set-frame-var-val ,new-frame
                                                      (nth ,param-num ,param-names)
                                                      ,param-choice
                                                      t))
...
CHOICES: (not evaluated) list, where either all choices are of the form
         (CHOICE-NAME CHOICE-FORMS) where CHOICE-NAME is not the symbol 'call, or all choices
         are of the form (CALL &rest ARGS) where ARGS would be the arguments to a CALL
         statement."
...
(defmacro dummy-choice (label)
  "macro dummy-choice LABEL
Set up a dummy choice point with label LABEL with the single choice labelled 'no-choice that
expands to nil."
...
(defun lookup-calisp-subroutine (f)
  (let ((l (get-lambda-list f)))
    (assert (notany
	     ...
	     l)
            ()
            "CALisp subroutines cannot have optional, key, rest, or aux arguments.")
    l))
...
Called by a thread to cause that thread's effectors to perform the given actions.  ACTION is
either a list of pairs, in which case it is treated as an association list from effectors to
their individual actions, or not, in which case it is treated as an individual action that is
done by each effector."
...
(defun act-crlm (c act label &aux (lock (lock c)))
  (with-process-lock (lock)
	...
	;; Wake up crlm thread either if all effectors have been assigned, or
	;; num-running-threads = 0 The second condition is for joint choices, and the first
	;; condition is necessary when not all threads possess effectors
	(dec-num-running-threads c)
...
ID is a symbol.
Look in the thread table of *crlm*, and return a list of the form (ID N) which does not name an
existing thread, where N is an integer."
  (list id
	(1+
	 (loop
...
WAIT-ACTION:    (evaluated) - when supplied, indicates that, if the destination thread has
                already committed to a joint action, then the effectors being reassigned will do
                this action (via a call to ACTION) on this step, and be reassigned at the
                beginning of the next step.  If this argument is not supplied, assert in the
                above situation."
  `(reassign-to-existing-thread *crlm* ',label ,@args))

(defun reassign-to-existing-thread (c label effectors id
                                    &key (wait-action nil wait-action-supplied)
                                    &aux (lock (lock c)))
  (with-process-lock (lock)
	...
	(gethash id (thread-states c))
      (assert exists ()
              #.(str "Attempted to reassign effectors ~A to nonexistent thread ~A.  "
                     "Actual thread list is ~A.") 
	...
	(waiting-to-act
	 (assert wait-action-supplied ()
                 #.(str "Attempted to reassign effectors ~A to thread ~A which is "
                        "already committed to an action, and the reassign call did "
                        "not specify a default action.")
                 effectors id)
	 (dolist (eff effectors)
...
(defun combined-state ()
  "combined state
Return combined state. Called within partial programs. Returned value should not be modified in
any way."
  (joint-state *crlm*))
...
(declaim (inline am-holding-lock wake-up-cpp-threads
                 dec-num-running-threads no-running-threads))
...
(defun lookup-process (c &optional (p *current-process*))
  "lookup-process CRLM &optional (PROCESS *CURRENT-PROCESS*)
Return 1) the thread id associated with PROCESS 2) the associated thread-state object"
...
(defun initialize-thread (c proc func args)
  "initialize-thread CRLM PROCESS FUNC ARGS

Initialize the process object PROCESS, so that once it is enabled it will begin by calling FUNC
with arguments ARGS, then call die.  Then enable the process."
...
(defun kill-all-threads (c)
  "kill-all-threads CRLM
Kill all currently existing cpp threads.  The stacks of the threads are unwound correctly."
...
(defun update-program-counter (t-state label)
  "update-program-counter T-STATE LABEL
Update the program counter of thread state to be at LABEL.  Uses the value of
*containing-subroutine-name* as the containing subroutine."
  (setf (ts-pc t-state)
...
(defun current-effectors (c)
  "current-effectors C
return a copy of the effector set of current environment state."
