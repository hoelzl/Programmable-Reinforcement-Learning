...
(defclass <threadwise-decomposed-q-learner> (<calisp-learning-algorithm> <q-learning-algorithm>)
  ...
  (:documentation
   "Class <threadwise-decomposed-q-learner> (<calisp-learning-algorithm> <q-learning-algorithm>)
...
(defmethod initialize-instance :after
    ((alg <threadwise-decomposed-q-learner>) &rest args &key q-function)
  (declare (ignore args))
	...
	(dolist (entry rewards)
	  (let ((acc-entry 
                  (check-not-null (assoc (car entry) reward-accs :test #'equal)
                                  "The reward accumulator entry for ~A in ~A"
                                  (car entry) reward-accs)))
  ...
  (with-slots (prev-omega prev-u total-discount q-function 
	...
	(let ((relevant-threads (list* id (filter 'list newly-spawned-threads
	  ...
	  (let ((target
	    ...
	    (debug-msg alg
                       "~&Did backup of component ~A towards target ~A.~&Weights are now ~A."
