...
(defclass <temporally-decomposed-q-learner>
    (<calisp-learning-algorithm> <q-learning-algorithm>)
  ((prev-omega :accessor prev-omega)
   ...
   (root-thread-id :initarg :root-thread-id :initform 'calisp-prog:root-thread
                   :reader root-thread-id)
  ...
  (:documentation
   "Class <temporally-decomposed-q-learner> (<calisp-learning-algorithm> <q-learning-algorithm>)
Represents a temporally (and threadwise) decomposed Q-learning algorithm for Concurrent ALisp
programs.
...
(defmethod initialize-instance :after
    ((alg <temporally-decomposed-q-learner>) &rest args &key q-function)
  (declare (ignore args))
...
(defstruct csi
  "Structure csi (choice-stack item).  
Contains bookkeeping information about previous choices made for which backups will be done for
a threadwise Q-function."
...
(defmethod inform-die-thread ((alg <temporally-decomposed-q-learner>) id)
  (let ((stack (evaluate (choice-stacks alg) id))
	...
	;; The case where the env terminates, causing the partial program to terminate A. For
	;; each choice-stack item
	(progn
	  (while stack
	    (let ((c (pop stack)))
	...
	(let ((current (first stack)))
	  (aif (csi-omega current)
               (let ((prev-u (csi-choice current)))
                 ;; if there's a previous state at this level do a set of backups depending on
                 ;; the type of statement and whether or not the thread has been stepped since
                 ;; then
                    ...
                    (let ((parent (evaluate-mv (newly-spawned alg) thread)))
                      (awhen parent
                        (let ((c (first (evaluate stacks it))))
                          (backup alg (csi-omega c) (csi-choice c) parent
                                  'qr (+ qr qc qe))))))))))))
...
(defun acc-last-state-at-level (current r)
  "acc-last-state-at-level CURRENT R
helper function for inform-env-step that increments the appropriate reward accumulator in
CURRENT, the csi at the lowest level of the thread, by R."
