...
(defun make-generosity-desc (gen-dist n)
  (make-2tbn-var-desc
   :id 'generosity
   :domain (make-instance '<var-set>
             :sets (loop repeat n collect (sample-space gen-dist)))
...
(defun make-taxi-2tbn (wm n
  ...
  "make-taxi-2tbn WORLD-MAP N 
               &key TRAFFIC-TRANS-DIST TRAFFIC-INIT-DIST TRAFFIC-SET SLIP-PROB COST-OF-LIVING
                    TOLL-FN TERM-PROB GEN-DIST SOURCES DESTS TAXI-START-POS

Create a 2tbn for the taxi example.  Params

WORLD-MAP:          2d boolean array in which position i,j is legal if the array value is true
...
SLIP-PROB:          the probability that an intended navigation move 'slips' resulting in a
                    perpendicular attempted move. .1 by default
...
DESTS:              set of places where passengers want to go.  by default is all legal
                    locations.
GEN-DIST:           distribution over generosity of a new passenger (must be an integer).
                    By default is always 5.
