...
(defun make-qe-taxi-smdp (2tbn n wm &optional (print-progress nil))
  ...
  (make-instance '<qe-taxi-smdp>
    :2tbn 2tbn :num-pass n
    :compute-state-set t  :print-progress print-progress
    :init-set (set:make-image-set (sample-space (init-dist 2tbn))
...
(defmethod mdp:avail-actions ((m <qe-taxi-smdp>) omega)
  (ecase (qe-js-stack omega)
    ((top serve-exit dropoff-action pickup-action get-pass put-pass nav-src nav-dest
          pass-choice-exit)
     '(no-choice))
    (nav-choice '(N E S W))
