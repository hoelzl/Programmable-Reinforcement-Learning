...
(defclass <stratagus-env> (<fully-observable-env>)
  ((game-socket :type socket-stream-internet-active
   ...
   (num-steps :type fixnum
	      :documentation 
              "Number of environment time steps (does not get reset when the environment state
is reset"
	      :accessor num-steps
	      :initform 0)
   (world        :writer set-world
		 :reader world
                 ;; FIXME: Can only be NIL because of the commented-out statement in
                 ;; INITIALIZE-INSTANCE below. --tc
                 :type (or null array))
   (player-id    :writer set-pid
		 :reader pid
		 :type fixnum))
  (:documentation "<stratagus-env> class. Initargs
:game-socket - a socket to the stratagus game.  Defaults to one connected to port 4870 of
localhost."))
...
(defmethod reset
    ((e <stratagus-env>) &optional (reset-to-nonterminal-state t) &aux (s (game-socket e)))
...
(defgeneric send (e m &optional check-ok)
  (:documentation "send E M &optional (CHECK-OK nil)
Send message M over the socket to the stratagus environment and wait for, and return a response.
If CHECK-OK is t, then additionally verify that the response is *ok-message*.  Could hang if the
connection is having issues and fails to respond.")
...
(defmethod get-game-state ((e <stratagus-env>) &aux (s (game-socket e)))  
  ...
  (let ((a (read s)))
    (until (not (symbolp a))
      ;; (assert (equal (symbol-name a) *ok-message*)
      ;;         () "Action for effector failed.  Response was ~a." a)
      (setf a (read s)))
...
(defgeneric compute-reward (e s a s-new)
  (:documentation
   "compute-reward (E <STRATAGUS-ENV>) (S strat-env-state) ACT (S-NEW strat-env-state)
...
(defmethod distance ((e <stratagus-env>) loc1 loc2 &optional (scale nil))
  "distance ENV LOC1 LOC2 &optional (SCALE nil)
For now, just return manhattan distance.  If SCALE is true, then divide by max distance"
